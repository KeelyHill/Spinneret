...
bynar_value * bynar_int_new(long long int int_val);
bynar_value * bynar_float_new(long double float_val);
...
bynar_value * bynar_list_append(bynar_value * list, bynar_value * value) {
 (__builtin_expect(!(list->type == bynar_list), 0) ? __assert_rtn(__func__, "./bynar.c", 142, "list->type == bynar_list") : (void)0);
...
bynar_value * bynar_dict_push(bynar_value * dict, bynar_value * key, bynar_value * value) {
 (__builtin_expect(!(dict->type == bynar_dict), 0) ? __assert_rtn(__func__, "./bynar.c", 195, "dict->type == bynar_dict") : (void)0);
...
bynar_value * bynar_dict_get_string(bynar_value * dict, const char * search_key) {
 (__builtin_expect(!(dict->type == bynar_dict), 0) ? __assert_rtn(__func__, "./bynar.c", 228, "dict->type == bynar_dict") : (void)0);

 for (int i=0; i < dict->u.dict.length; i++) {

   if (!strncmp(
    dict->u.dict.values[i].key->u.string.pointer, search_key,
                dict->u.dict.values[i].key->u.string.length)
      ) {
...
bynar_value * bynar_string_new(const char * string_value_buf, size_t length) {
 ...
 __builtin___memcpy_chk (copy, string_value_buf, length * sizeof(char), __builtin_object_size (copy, 0));
...
void bynar_serialize(char * out_buf, bynar_value * value) {
 ...
 while (value) {
  switch (value->type) {
   ...
   case bynar_int:
    out_buf += __builtin___sprintf_chk (out_buf, 0, __builtin_object_size (out_buf, 2 > 1 ? 1 : 0), "i%lli;", value->u.integer);
    break;

   case bynar_float:

    out_buf += __builtin___sprintf_chk (out_buf, 0, __builtin_object_size (out_buf, 2 > 1 ? 1 : 0), "f%Lg;", value->u.flt);
    break;

   case bynar_bool:
    *out_buf++ = value->u.boolean ? 'T' : 'F';
    break;
   ...
   case bynar_string:
    ...
    out_buf += __builtin___sprintf_chk (out_buf, 0, __builtin_object_size (out_buf, 2 > 1 ? 1 : 0), "s%u:", value->u.string.length);
   ...
   case bynar_list:
    ...
    if (((bynar_sequence_builder *) value)->iter_count == 0) {
    ...
    if (((bynar_sequence_builder *) value)->iter_count == value->u.list.length) {
     *out_buf++ = ';';
     break;
    }

    ((bynar_sequence_builder *) value)->iter_count += 1;
    value = value->u.list.values[((bynar_sequence_builder *) value)->iter_count-1];
   ...
   case bynar_dict:
    ...
    if (((bynar_sequence_builder *) value)->iter_count == value->u.dict.length * 2) {
     *out_buf++ = ';';
     break;
    }
    ...
    if (((bynar_sequence_builder *) value)->iter_count & 1)
     value = value->u.dict.values[((bynar_sequence_builder *) value)->iter_count/2].key;
    else
     value = value->u.dict.values[(((bynar_sequence_builder *) value)->iter_count-1)/2].value;
...
size_t bynar_get_serialized_size(bynar_value * value) {
 ...
 while (value) {
  ...
  switch (value->type) {
   ...
   case bynar_int:
    total += __builtin___snprintf_chk (((void *)0), 0, 0, __builtin_object_size (((void *)0), 2 > 1 ? 1 : 0), "i%lli;", value->u.integer);
    break;
   case bynar_float:

    total += __builtin___snprintf_chk (((void *)0), 0, 0, __builtin_object_size (((void *)0), 2 > 1 ? 1 : 0), "f%Lg;", value->u.flt);
    break;
   case bynar_bool:
   case bynar_null:
    total += 1;
   ...
   case bynar_string:
    ...
    total += __builtin___snprintf_chk (((void *)0), 0, 0, __builtin_object_size (((void *)0), 2 > 1 ? 1 : 0), "s%u:", value->u.string.length);
   ...
   case bynar_list:
    ...
    if (((bynar_sequence_builder *) value)->iter_count == value->u.list.length) {
     total += 1;
     ((bynar_sequence_builder *) value)->iter_count = 0;
     break;
    }
 ...
 for(int i=0; i<len; ++i) {
  ...
  switch (c) {
   case 'i':
    ...
    if (!found) {
     __builtin___sprintf_chk (error_buf, 0, __builtin_object_size (error_buf, 2 > 1 ? 1 : 0), "Expected `;` near %i to end int", i);
     return ((void *)0);
    }
   ...
   case 'f':
    ...
    if (!found) {
     __builtin___sprintf_chk (error_buf, 0, __builtin_object_size (error_buf, 2 > 1 ? 1 : 0), "Expected `;` near %i to end float", i);
     return ((void *)0);
    }
    i+=1;
   ...
   case 's':
    ...
    if (!found) {
     __builtin___sprintf_chk (error_buf, 0, __builtin_object_size (error_buf, 2 > 1 ? 1 : 0), "Expected `:` after %i to start string", i);
     return ((void *)0);
    }
...
# 10 "./barebone-spinneret-node.c" 2
...
void str_cpy_terminated(char *restrict dst, const char *restrict src, size_t n) {
 __builtin___strncpy_chk (dst, src, n, __builtin_object_size (dst, 2 > 1 ? 1 : 0));
 dst[n] = '\0';
}
...
struct this_node {
 ...
 bynar_value * secure_groups;
 bynar_value * joined_groups;
} me_node;
...
void encode_broadcast(broadcast_t *b , transmittable_broadcast_t *tb );
...
int handle_raw_broadcast(const char *raw_data, transmittable_broadcast_t *tb) {
 ...
 if (raw_data[0] == '\x01') {
  ...
  if (( (*(char*)&i) == 0 )) { endian[0] = raw_data[2]; endian[1] = raw_data[3]; }
...
int process_plain_broadcast(const char * plaindata, transmittable_broadcast_t *tb, unsigned short len) {
 ...
 short one_more_section = 0;

 num = strchr(buf, '|') - buf;
 if (num<=0) num = &plaindata[len] - buf;
 else one_more_section = 1;
 ...
 for (int i=0; i < parsed->payload_len; i++) {
  ...
  if (buf[i] == '^') {
   ...
   if (args_len == 1) {
    ...
    args_list = bynar_deserialize(&buf[i], args_len+1, ((void *)0));
   }

   i += args_len;

   this_value = (* me_node.do_action_ptr)(act_name, args_list);

   this_key = bynar_string_new_nocopy(act_name, name_len);
  ...
  case REQ:
   {
    ...
    char * payload_serialized = malloc(serialized_size);
    ...
    broadcast_t brocast_to_build = {RESP, parsed->from, parsed->to, payload_serialized, (uint16_t)serialized_size, ((void *)0)};
...
char * buffer_append_str_len(char *buf, char *str, size_t len) {
...
void encode_broadcast(broadcast_t *b , transmittable_broadcast_t *tb ) {
 ...
 if (b->payload[0] == '#') {
  ...
  const char * group_name = b->payload + 1;
  bynar_value * group_key = ((void *)0);

  group_key = bynar_dict_get_string(me_node.secure_groups, group_name);

  if (group_key != ((void *)0)) {

   encrypted_payload = (char *) malloc(b->payload_len );



   unsigned char * nonce;
   randombytes(nonce, 12);
   buffer_append_str_len(encrypted_payload, 12);
 ...
 *out++ = '|';
 ...
 switch (b->kind) {
  case REQ:
   ...
   out = buffer_append_str_len(out, b->payload, b->payload_len);
...
int main(void) {
 ...
 handle_raw_broadcast("\x01\x00\x00\x38nanonc|REQ|tofoo|frmbar|c3RhdGUsXmZ1bmMoczQ6KGhpKSk=", &tb);
# 60 "main.c"
 stop=clock();
...
# 1 "randombytes/devurandom.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 329 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "randombytes/devurandom.c" 2
